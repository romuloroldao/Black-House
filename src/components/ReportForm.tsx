...
const ReportForm = ({ reportId, onSuccess, onCancel }: ReportFormProps) => {
  ...
  const loadAlunos = async () => {
    try {
      ...
      toast.error("Erro ao carregar alunos: " + (error.message || String(error)));
    }
  };
  ...
  const loadAlunoPhotos = async () => {
    ...
    try {
      ...
      setAlunoPhotos(Array.isArray(data) ? data : []);
    } catch (error: any) {
      console.error("Erro ao carregar fotos:", error);
      toast.error("Erro ao carregar fotos: " + (error.message || String(error)));
    }
  };
  ...
  const loadReport = async () => {
    ...
    try {
      ...
      const relatorio = Array.isArray(data) && data.length > 0 ? data[0] : null;
      ...
      const midiasData = await apiClient
        .from("relatorio_midias")
        .select("url")
        .eq("relatorio_id", reportId);

      const midias = Array.isArray(midiasData) ? midiasData : [];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (selectedPhotos.length > 0 && currentReportId) {
        // Inserir fotos uma por uma (apiClient não suporta batch insert)
