...
const StudentFinancialManagement = ({ studentId, studentName }: Props) => {
  ...
  const loadFinancialData = async () => {
    try {
      ...
      // Carregar pagamentos
      const pagamentosData = await apiClient
        .from("asaas_payments")
        .select("*")
      ...
      // DESIGN-022: Carregamento de payment_plans removido - não há API semântica disponível
      // Requer DESIGN-019 para implementação futura
      setPlanos([]);
      
      // Carregar exceções financeiras
      const excecoesData = await apiClient
        .from("financial_exceptions")
        .select("*")
        .eq("aluno_id", studentId)
        .eq("ativo", true);
      
      setExcecoes(Array.isArray(excecoesData) ? excecoesData : []);
      ...
      let configData = Array.isArray(configs) && configs.length > 0 ? configs[0] : null;
      
      // DESIGN-022: Busca de payment_plans removida - não há API semântica disponível
      // Se configData.payment_plan_id existir, manter sem dados do plano
      // Requer DESIGN-019 para implementação futura
  ...
  const handleCreatePayment = async () => {
    try {
      // Chamar endpoint local ao invés de Edge Function
      const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001';
      const token = apiClient.getToken();
      
      const response = await fetch(`${API_URL}/api/payments/create-asaas`, {
        method: 'POST',
        headers: {
  ...
  const handleSaveRecurring = async () => {
    try {
      ...
      if (configCobranca) {
        await apiClient
          .from("recurring_charges_config")
          .update({ ...data, id: configCobranca.id });
      } else {
        await apiClient
          .from("recurring_charges_config")
          .insert(data);
  ...
  const handleRemoveRecurring = async () => {
    try {
      ...
      await apiClient
        .from("recurring_charges_config")
        .update({ ativo: false, id: configCobranca.id });
