...
const DietCreator = ({ dietaId }: DietCreatorProps) => {
  ...
  const carregarDados = async () => {
    try {
      ...
      setAlimentos(Array.isArray(alimentosRes) ? alimentosRes : []);
  ...
  const carregarDietaExistente = async () => {
    ...
    try {
      ...
      const farmacosData = Array.isArray(farmacosArray) ? farmacosArray : [];
  ...
  const calcularSubstituicoes = (item: ItemRefeicao): Array<{nome: string, quantidade: number, nutriente: string}> => {
    ...
    const quantidadeRef = typeof alimento.quantidade_referencia_g === 'string' 
      ? parseFloat(alimento.quantidade_referencia_g) || 100 
      : (alimento.quantidade_referencia_g || 100);
    const kcalRef = typeof alimento.kcal_por_referencia === 'string' 
    ...
    return alimentos
      ...
      .map(sub => {
        ...
        const subKcalRef = typeof sub.kcal_por_referencia === 'string' 
          ? parseFloat(sub.kcal_por_referencia) || 0 
          : (sub.kcal_por_referencia || 0);
        const subQtdRef = typeof sub.quantidade_referencia_g === 'string' 
          ? parseFloat(sub.quantidade_referencia_g) || 100 
          : (sub.quantidade_referencia_g || 100);
  ...
  const calcularTotaisRefeicao = (refeicao: Refeicao) => {
    return refeicao.itens.reduce((total, item) => {
      ...
      const quantidade = typeof item.quantidade === 'string' ? parseFloat(item.quantidade) || 0 : (item.quantidade || 0);
      const quantidadeRef = typeof item.alimento.quantidade_referencia_g === 'string' 
        ? parseFloat(item.alimento.quantidade_referencia_g) || 100 
        : (item.alimento.quantidade_referencia_g || 100);
      
      const fator = quantidade / quantidadeRef;
      
      // Garantir que valores nutricionais sejam números
      const kcal = typeof item.alimento.kcal_por_referencia === 'string' 
        ? parseFloat(item.alimento.kcal_por_referencia) || 0 
        : (item.alimento.kcal_por_referencia || 0);
      const ptn = typeof item.alimento.ptn_por_referencia === 'string' 
        ? parseFloat(item.alimento.ptn_por_referencia) || 0 
        : (item.alimento.ptn_por_referencia || 0);
      const cho = typeof item.alimento.cho_por_referencia === 'string' 
        ? parseFloat(item.alimento.cho_por_referencia) || 0 
        : (item.alimento.cho_por_referencia || 0);
      const lip = typeof item.alimento.lip_por_referencia === 'string' 
        ? parseFloat(item.alimento.lip_por_referencia) || 0 
        : (item.alimento.lip_por_referencia || 0);
  ...
  const salvarDieta = async () => {
    ...
    try {
      ...
      if (!dietaIdAtual) {
        ...
        dietaIdAtual = Array.isArray(dieta) && dieta.length > 0 ? dieta[0].id : null;
        ...
        // Buscar IDs primeiro
        const itensAntigos = await apiClient
          .from('itens_dieta')
          .select('id')
          .eq('dieta_id', dietaIdAtual);
        const farmacosAntigos = await apiClient
          .from('dieta_farmacos')
          .select('id')
          .eq('dieta_id', dietaIdAtual);
        ...
        if (Array.isArray(itensAntigos)) {
          for (const item of itensAntigos) {
            ...
            await apiClient.request(`/api/itens-dieta/${item.id}`, { method: 'DELETE' });
          }
        }
        if (Array.isArray(farmacosAntigos)) {
          for (const farmaco of farmacosAntigos) {
            // DESIGN-SUPABASE-PURGE-GLOBAL-003: Usar rota semântica ao invés de from()
            await apiClient.request(`/api/dieta-farmacos/${farmaco.id}`, { method: 'DELETE' });
          }
        }
      }
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center h-10 px-3 border rounded-md bg-muted">
                          {item.alimento ? (() => {
                            ...
                            const kcalRef = typeof item.alimento.kcal_por_referencia === 'string' 
                              ? parseFloat(item.alimento.kcal_por_referencia) || 0 
                              : (item.alimento.kcal_por_referencia || 0);
                            const qtdRef = typeof item.alimento.quantidade_referencia_g === 'string' 
                              ? parseFloat(item.alimento.quantidade_referencia_g) || 100 
                              : (item.alimento.quantidade_referencia_g || 100);
