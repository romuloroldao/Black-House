...
const MessageManager = () => {
  ...
  const carregarConversas = async () => {
    ...
    try {
      const conversasData = await apiClient
        .from("conversas")
        .select("id, aluno_id, ultima_mensagem, ultima_mensagem_em, updated_at")
        .eq("coach_id", user.id)
        .order("updated_at", { ascending: false });

      // Buscar nomes dos alunos e contar mensagens não lidas
      const conversasComNomes = await Promise.all(
        (Array.isArray(conversasData) ? conversasData : []).map(async (conversa) => {
          try {
            const alunos = await apiClient
              .from("alunos")
              .select("nome")
              .eq("id", conversa.aluno_id);
            const alunoData = Array.isArray(alunos) && alunos.length > 0 ? alunos[0] : null;
            ...
            const count = Array.isArray(mensagens) ? mensagens.length : 0;
  ...
  const carregarMensagens = async (conversaId: string) => {
    try {
      ...
      if (user) {
        ...
        const mensagensNaoLidas = await apiClient
          .from("mensagens")
          .select("id")
          .eq("conversa_id", conversaId)
          .neq("remetente_id", user.id)
        ...
        // Atualizar cada uma
        if (Array.isArray(mensagensNaoLidas)) {
          for (const msg of mensagensNaoLidas) {
            await apiClient
              .from("mensagens")
              .update({ lida: true, id: msg.id });
          }
        }
      }
    } catch (error) {
  ...
  const iniciarNovaConversa = async (alunoId: string) => {
    ...
    try {
      ...
      const conversasExistentes = await apiClient
        .from("conversas")
        .select("id")
        .eq("coach_id", user.id)
        .eq("aluno_id", alunoId);
      
      const conversaExistente = Array.isArray(conversasExistentes) && conversasExistentes.length > 0 ? conversasExistentes[0] : null;
      ...
      if (conversaExistente) {
        ...
        const novaConversa = Array.isArray(data) && data.length > 0 ? data[0] : null;
        if (!novaConversa) throw new Error('Erro ao criar conversa');
