...
const StudentManager = () => {
  ...
  // useEffect só executa quando isReady === true
  useEffect(() => {
    if (!isReady) return;
    
    // DESIGN-FRONTEND-POST-IMPORT-SEMANTIC-DATA-REFRESH-018: Apenas carregar dados com rotas semânticas
    carregarAlunos();
    // carregarPlanos() removido - não há rota semântica disponível
    // DESIGN-FRONTEND-POST-IMPORT-SEMANTIC-DATA-REFRESH-018: payment_plans requer rota semântica futura
    carregarCoach();
  }, [isReady]);
  ...
  // DESIGN-FRONTEND-POST-IMPORT-SEMANTIC-DATA-REFRESH-018: carregarPlanos() removido
  // Não há rota semântica para payment_plans disponível
  // Requer DESIGN-PAYMENT-PLANS-SEMANTIC-API-019 para implementação futura
  // Por enquanto, paymentPlans permanece vazio ou pode ser preenchido via props/contexto
  // const carregarPlanos = async () => {
  //   // Removido - violava DESIGN-SUPABASE-PURGE-GLOBAL-003
  // };
  ...
  const carregarAlunos = async () => {
    try {
      ...
      // DESIGN-FRONTEND-IMPORT-POSTGREST-PURGE-016: Usar rota semântica ao invés de .from()
      // DESIGN-FRONTEND-DATA-CONTEXT-LOCK-015: getAlunosByCoach() já tem guard de contexto
      const data = await apiClient.getAlunosByCoach();

      // Get emails of all users with 'coach' role to exclude them from student list
      // RPCs Supabase removidas; manter lista vazia por enquanto
      let coachEmails: string[] = [];

      // Filter out any alunos whose email matches a coach's email
      // A resposta de getAlunosByCoach() já vem como array
      let filteredData = (Array.isArray(data) ? data : []).filter(aluno => 
      ...
      // DESIGN-FRONTEND-DATA-CONTEXT-LOCK-015: Tratar erros de contexto
      if (error?.code === 'DATA-CONTEXT-NOT-INITIALIZED' || error?.code === 'DATA-CONTEXT-NO-IDENTITY') {
        console.warn('[DESIGN-FRONTEND-DATA-CONTEXT-LOCK-015] Contexto não inicializado. Aguardando...');
        // Não definir loading como false para manter o estado de carregamento
        return;
      }
      console.error('Erro ao carregar alunos:', error);
      toast({
        title: "Erro ao carregar alunos",
        description: error?.message || "Não foi possível carregar a lista de alunos",
        variant: "destructive",
      });
    } finally {
      setLoading(false);
  ...
  const handleSaveStudent = async () => {
    try {
      ...
      if (editingStudent) {
        ...
        // Insert new student
        const insertedData = await apiClient
          .from('alunos')
          .insert({
            nome: newStudent.nome,
            email: newStudent.email,
        ...
        alunoId = Array.isArray(insertedData) && insertedData.length > 0 ? insertedData[0].id : null;
      ...
      if (newStudent.plano && alunoId) {
        ...
        try {
          const configs = await apiClient
            ...
            .select('id')
            .eq('aluno_id', alunoId);
          existingConfig = Array.isArray(configs) && configs.length > 0 ? configs[0] : null;
        } catch (error) {
          console.warn('Erro ao verificar configuração existente:', error);
        }
  ...
  const handleEditStudent = async (student: Student) => {
    ...
    let diaCobranca = "";
    try {
      const configDataArray = await apiClient
        .from('recurring_charges_config')
        .select('dia_vencimento_customizado, payment_plan_id')
        .eq('aluno_id', student.id)
        .eq('ativo', true);
      
      const configData = Array.isArray(configDataArray) && configDataArray.length > 0 ? configDataArray[0] : null;
      ...
      console.warn('Erro ao buscar configuração de cobrança:', error);
    }
