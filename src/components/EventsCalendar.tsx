...
export function EventsCalendar() {
  ...
  const loadEventos = async () => {
    ...
    try {
      ...
      const eventosComRelacoes = await Promise.all(
        (Array.isArray(data) ? data : []).map(async (evento) => {
          // Buscar turma
          let turma = null;
          if (evento.turma_id) {
            const turmas = await apiClient
              .from("turmas")
              .select("nome")
              .eq("id", evento.turma_id);
            turma = Array.isArray(turmas) && turmas.length > 0 ? turmas[0] : null;
          }

          // Buscar participantes
          const participantes = await apiClient
            .from("eventos_participantes")
            .select("aluno_id")
            .eq("evento_id", evento.id);

          const participantesComAlunos = await Promise.all(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const eventoData = Array.isArray(evento) && evento.length > 0 ? evento[0] : null;
    if (!eventoData) {
    ...
    if (participantesIds.length > 0) {
      ...
      await apiClient.request('/api/eventos-participantes', {
        method: 'POST',
