...
export default function PlanManager() {
  ...
  const loadData = async () => {
    try {
      ...
      const [plansRes, alunosRes, configsRes] = await Promise.all([
        ...
        apiClient.request('/api/payment-plans'),
        ...
        // DESIGN-SUPABASE-PURGE-GLOBAL-003: Usar rota semântica ao invés de from()
        apiClient.request('/api/recurring-charges-config?ativo=true')
      ]);

      // Buscar dados dos alunos para cada config separadamente
      const configsComAlunos = await Promise.all(
        (Array.isArray(configsRes) ? configsRes : []).map(async (config) => {
          if (config.aluno_id) {
            ...
            return { ...config, aluno: aluno || null };
          }
          return { ...config, aluno: null };
        })
      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingPlan) {
        await apiClient
          .from("payment_plans")
          .update({ ...planData, id: editingPlan.id });
  ...
  const handleRemoveStudent = async (configId: string) => {
    ...
    try {
      await apiClient
        .from("recurring_charges_config")
        .update({ ativo: false, id: configId });
