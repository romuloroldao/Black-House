...
interface FoodSubstitutionDialogProps {
  ...
  alimentoAtual: Alimento | null;
  quantidadeAtual: number;
  alimentosDisponiveis: Alimento[];
...
export default function FoodSubstitutionDialog({
  ...
  const calcularSubstituicoes = () => {
    ...
    const alimentosDoMesmoGrupo = alimentosDisponiveis.filter(
      (alimento) =>
        alimento.tipo_id === alimentoAtual.tipo_id &&
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <Badge variant="outline" className="text-xs">
                          {sub.alimento.origem_ptn}
                        </Badge>
                      </div>
                      <p className="text-xs text-muted-foreground">{sub.formula}</p>
                      <div className="flex gap-4 text-xs">
                        <span>
                          {((sub.alimento.kcal_por_referencia / sub.alimento.quantidade_referencia_g) * sub.quantidadeEquivalente).toFixed(1)} kcal
                        </span>
                        <span>
                          CHO: {((sub.alimento.cho_por_referencia / sub.alimento.quantidade_referencia_g) * sub.quantidadeEquivalente).toFixed(1)}g
                        </span>
                        <span>
                          PTN: {((sub.alimento.ptn_por_referencia / sub.alimento.quantidade_referencia_g) * sub.quantidadeEquivalente).toFixed(1)}g
                        </span>
                      </div>
                    </Label>
                  </div>
