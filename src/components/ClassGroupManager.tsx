...
export function ClassGroupManager() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (selectedTurma) {
        await apiClient
          .from("turmas")
          .update({ ...formData, id: selectedTurma.id });
  ...
  const handleRemoveMember = async (alunoId: string) => {
    ...
    try {
      await apiClient
        .from("turmas_alunos")
        .delete({ turma_id: selectedTurma.id, aluno_id: alunoId });
