...
export default function FinancialDashboard() {
  ...
  const loadFinancialData = async () => {
    try {
      ...
      const paymentsData = await apiClient
        .from("asaas_payments")
        .select("*")
        .eq("coach_id", user?.id)
      ...
      const paymentsComAlunos = await Promise.all(
        (Array.isArray(paymentsData) ? paymentsData : []).map(async (payment) => {
          ...
          return { ...payment, alunos: null };
        })
      );

      const expensesData = await apiClient
        .from("expenses")
        .select("*")
        .eq("coach_id", user?.id)
  ...
  const loadPaymentData = async () => {
    try {
      ...
      const payments = await apiClient
        .from("asaas_payments")
        .select("*")
        .eq("coach_id", user?.id)
        .order("due_date", { ascending: false });

      // Buscar dados dos alunos para cada pagamento
      const paymentsComAlunos = await Promise.all(
        (Array.isArray(payments) ? payments : []).map(async (payment) => {
          if (payment.aluno_id) {
            const alunosData = await apiClient
              .from("alunos")
              .select("nome, email")
              .eq("id", payment.aluno_id);
            const aluno = Array.isArray(alunosData) && alunosData.length > 0 ? alunosData[0] : null;
            return {
              ...payment,
