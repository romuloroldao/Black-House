...
const PaymentManager = () => {
  ...
  const loadData = async () => {
    try {
      ...
      // Carregar planos
      const plansData = await apiClient
        .from("payment_plans")
        .select("*")
        .eq("coach_id", user?.id)
        .eq("ativo", true)
  ...
  const loadPayments = async () => {
    try {
      ...
      const paymentsWithNames = await Promise.all(
        (Array.isArray(data) ? data : []).map(async (payment) => {
          try {
            const alunoDataArray = await apiClient
              .from("alunos")
              .select("nome")
              .eq("id", payment.aluno_id);
            
            const alunoData = Array.isArray(alunoDataArray) && alunoDataArray.length > 0 ? alunoDataArray[0] : null;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Chamar endpoint da API própria (substitui Edge Function do Supabase)
      const API_URL = import.meta.env.VITE_API_URL || 'http://localhost:3001';
      const token = apiClient.getToken();
      
      const response = await fetch(`${API_URL}/functions/create-asaas-payment`, {
        method: 'POST',
        headers: {
