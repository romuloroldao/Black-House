...
const ERROR_MESSAGES = {
    ...
    [ErrorType.BACKEND]: 'Erro de autenticação.',
...
class ApiClient {
    ...
    private async request(endpoint: string, options: RequestInit = {}) {
        ...
        try {
            ...
            if (!response.ok) {
                ...
                (backendError as any).errorType = ErrorType.BACKEND;
            ...
            if (error instanceof Error && (error as any).errorType === ErrorType.BACKEND) {
                throw error;
            }
    ...
    // Database queries
    // DESIGN-SUPABASE-PURGE-GLOBAL-003: Sintaxe PostgREST é FORBIDDEN
    // DESIGN-FRONTEND-DATA-CONTEXT-LOCK-015: from() também viola lock de contexto
    // Este método foi REMOVIDO e agora lança erro explícito
    // Use specific API endpoints like getAlunosByCoach(), getNotifications(), getMe(), etc.
    // NÃO USE apiClient.from() - Use rotas semânticas específicas
    from(table: string) {
        assertNoSupabaseDirectAccess('from()');
        const error = new Error(
            `DESIGN-SUPABASE-PURGE-GLOBAL-003 + DESIGN-FRONTEND-DATA-CONTEXT-LOCK-015: ` +
            `apiClient.from('${table}') é FORBIDDEN. ` +
    ...
    async uploadFile(bucket: string, path: string, file: File) {
        try {
            // Para avatares, usar nova rota /api/uploads/avatar
            if (bucket === 'avatars') {
    ...
    async getNotifications(options?: { lida?: boolean; tipo?: string; limit?: number }) {
        assertDataContextReady('getNotifications()');
    ...
    async getProfile() {
        assertDataContextReady('getProfile()');
